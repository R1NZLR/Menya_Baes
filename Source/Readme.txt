			...
			It's using namespaces cuz hiding sh*t doesn't stick, and doing it in header files f*cks the compile up.
				CREDITS: Alejandro [ScriptHook V], MAFINS [Menya Baes], 2much4u [original xsc menu scroll]. 


// Keep in mind that wherever I say 'string', I actually mean char or char*.

Bugs:
Option Breaks (they're a thing) are counted as options. So they'll be in the option count. I tried fixing it using l33t hax
...
VectorToFloat << Store the values of a Vector3 (struct) in an array : [1st: Vector3] & [2nd: Array];
...
Check_compare_string_length << Check the length of one string with an amount. Retuns true if less that or equal to the amount/range : [1st = string] & [2nd = max size];
